...
const YourCommunity = () => {
    ...
    const {img} = community.img;
    const {logo} = community.logo;
    const {description} = community.description;
    const {contact} = community.contact;
    const {president} = community.president;
    ...
    return (
        <SafeAreaView style={{flex: 1}}>
            ...
            <ImageBackground source={require("../../../assets/volunteer/bg3.png")}
                ...
                <ScrollView>
                    <Header3 headerText={community.name} logo2={community.logo} />
                    <View style={styles.about}>
                        ...
                        <View style={{backgroundColor:"#D9D9D9", padding:10, margin:10, borderRadius:20}}>
                            <Text style={{fontWeight:"bold"}}>President:        {community.president}</Text>
                            <Text style={{fontWeight:"bold"}}>Contact:            {community.contact}</Text>
                            <Text style={{fontWeight:"bold"}}>Total Events:    {community.events}</Text>
                        </View>


                    </View>
                    <Image source={{uri:community.img}} style={{marginLeft:30, height:200, width:350}}></Image>
                    ...
                    <View style={{flexDirection: "row"}}>
                        <TouchableOpacity
                            style={styles.joine}
                            onPress={handleJoin}>
                            <Text style={{fontSize: 20, color: "#F3AF4A", fontWeight: "bold", marginLeft: 15}}>Events Page</Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                            style={styles.joine1}
